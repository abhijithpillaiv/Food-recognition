...
export default function foodrec({ carb, fat, protein, setcarb, calories, setfat, setprotein, setcalories }) {
    ...
    useEffect(() => {
        if (nutretionData[rec]) {
            setcarb(carb + nutretionData[rec]['Carbohydrate'])
            setfat(fat + nutretionData[rec]['fat'])
            setprotein(protein + nutretionData[rec]['protein'])
            setcalories(calories + nutretionData[rec]['calories'])
        }
    }, [rec]);
    return (
        <div className="container-fluid" data-aos="fade-up">
            <div className="row">
                <div style={{ textAlign: 'center' }} >
                    <div style={{ padding: '0px' }}>
                        ...
                        <div style={{ paddingTop: '10px' }}>
                            ...
                            <div>
                                <button onClick={handleUploadImage} className='btn btn-primary'>Add food</button>
                            </div>
                        </div>
                ...
                <div style={{ textAlign: 'center', paddingTop: '10px' }}>
                    <span style={{textDecoration:'underline'}}>{rec}</span>
                    <table >
                        <tbody >
                            {nutretionData[rec] ? <>
                                <tr >
                                    <td>Carbohydrate</td>
                                    <td />
                                    <td>: {nutretionData[rec]['Carbohydrate']}</td>
                                    <td /><td />
                                    <td>Fat</td>
                                    <td />
                                    <td>: {nutretionData[rec]['fat']}</td>
                                </tr>
                                <tr>
                                    <td>Protein</td>
                                    <td />
                                    <td>: {nutretionData[rec]['protein']}</td>
                                    <td /><td />
                                    <td> Calories</td>
                                    <td />
                                    <td>: {nutretionData[rec]['calories']}</td>
                                </tr>
                            </>
