...
def predict_class(model, images, show = True):
  ...
  # img = image.load_img('d.jpeg', target_size=(299, 299))
  # img = image.img_to_array(img)                    
  # img = np.expand_dims(img, axis=0)         
  # img /= 255.                                      
...
def getTestData(path):
  with open(path,'r') as txt:
    images=['/media/black/data/work/mini/backend/food-101/images/'+read.strip()+'.jpg' for read in txt.readlines()]
    print(images)
    return images


print("PREDICTIONS BASED ON PICTURES UPLOADED")
images=getTestData('food-101/meta/test.txt')
